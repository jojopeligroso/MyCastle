import { pgTable, char, text, index, uniqueIndex, foreignKey, unique, check, uuid, date, timestamp, integer, numeric, pgPolicy, varchar, jsonb, boolean, time, pgView, bigint, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const attendanceStatus = pgEnum("attendance_status", ['present', 'absent', 'late', 'excused'])
export const paymentMethod = pgEnum("payment_method", ['cash', 'card', 'bank_transfer', 'online', 'other'])
...
export const learner = pgTable("learner", {
	...
	index("ix_learner_name").using("btree", table.lastName.asc().nullsLast().op("text_ops"), table.firstName.asc().nullsLast().op("text_ops")),
	uniqueIndex("uq_learner_identity").using("btree", table.firstName.asc().nullsLast().op("bpchar_ops"), table.lastName.asc().nullsLast().op("bpchar_ops"), table.dob.asc().nullsLast().op("bpchar_ops"), table.citizenshipCode.asc().nullsLast().op("date_ops")),
	foreignKey({
...
export const booking = pgTable("booking", {
	...
	accommodationFee: numeric("accommodation_fee", { precision: 12, scale:  2 }).default('0').notNull(),
	...
	registrationFee: numeric("registration_fee", { precision: 12, scale:  2 }).default('0').notNull(),
	...
	totalBooking: numeric("total_booking", { precision: 12, scale:  2 }).generatedAlwaysAs(sql`(((((((deposit_paid + course_fee) + accommodation_fee) + transfer_fee) + exam_fee) + registration_fee) + learner_prot_fee) + medical_ins_fee)`),
	totalDue: numeric("total_due", { precision: 12, scale:  2 }).generatedAlwaysAs(sql`(((((((course_fee + accommodation_fee) + transfer_fee) + exam_fee) + registration_fee) + learner_prot_fee) + medical_ins_fee) - deposit_paid)`),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (table) => [
...
export const payments = pgTable("payments", {
	...
	paidOn: timestamp("paid_on", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	reference: text(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (table) => [
...
export const programmes = pgTable("programmes", {
	...
	index("idx_programmes_deleted").using("btree", table.deletedAt.asc().nullsLast().op("timestamp_ops")).where(sql`(deleted_at IS NULL)`),
	...
	pgPolicy("programmes_delete_admin", { as: "permissive", for: "delete", to: ["public"], using: sql`((tenant_id = (current_setting('app.current_tenant_id'::text))::uuid) AND (current_setting('app.current_user_role'::text) = ANY (ARRAY['admin'::text, 'super_admin'::text])))` }),
...
export const accommodation = pgTable("accommodation", {
	...
	index("ix_accommodation_booking").using("btree", table.bookingId.asc().nullsLast().op("uuid_ops")),
	index("ix_accommodation_window").using("btree", table.startDate.asc().nullsLast().op("date_ops"), table.endDate.asc().nullsLast().op("date_ops")),
	foreignKey({
			columns: [table.bookingId],
			foreignColumns: [booking.id],
...
export const attendanceDay = pgTable("attendance_day", {
	id: uuid().default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	courseId: uuid("course_id").notNull(),
...
export const attendanceMark = pgTable("attendance_mark", {
	id: uuid().default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	...
	unique("attendance_mark_attendance_day_id_learner_id_key").on(table.attendanceDayId, table.learnerId),
]);
...
export const courses = pgTable("courses", {
	...
	index("idx_courses_deleted").using("btree", table.deletedAt.asc().nullsLast().op("timestamp_ops")).where(sql`(deleted_at IS NULL)`),
	index("idx_courses_programme").using("btree", table.programmeId.asc().nullsLast().op("uuid_ops")),
	...
	pgPolicy("courses_delete_admin", { as: "permissive", for: "delete", to: ["public"], using: sql`((tenant_id = (current_setting('app.current_tenant_id'::text))::uuid) AND (current_setting('app.current_user_role'::text) = ANY (ARRAY['admin'::text, 'super_admin'::text])))` }),
	...
	check("check_cefr_level", sql`(cefr_level)::text = ANY ((ARRAY['A1'::character varying, 'A2'::character varying, 'B1'::character varying, 'B2'::character varying, 'C1'::character varying, 'C2'::character varying])::text[])`),
]);
...
export const users = pgTable("users", {
	...
	index("idx_users_current_level").using("btree", table.currentLevel.asc().nullsLast().op("text_ops")).where(sql`((role)::text = 'student'::text)`),
	index("idx_users_level_status").using("btree", table.levelStatus.asc().nullsLast().op("text_ops")).where(sql`((role)::text = 'student'::text)`),
	...
	uniqueIndex("idx_users_tenant_email").using("btree", table.tenantId.asc().nullsLast().op("uuid_ops"), table.email.asc().nullsLast().op("text_ops")),
	index("idx_users_visa_expiry").using("btree", table.visaExpiry.asc().nullsLast().op("date_ops")).where(sql`(((role)::text = 'student'::text) AND (visa_expiry IS NOT NULL))`),
	foreignKey({
			columns: [table.tenantId],
			foreignColumns: [tenants.id],
			name: "users_tenant_id_fkey"
		}),
	check("check_current_level", sql`(current_level IS NULL) OR ((current_level)::text = ANY ((ARRAY['A1'::character varying, 'A2'::character varying, 'B1'::character varying, 'B2'::character varying, 'C1'::character varying, 'C2'::character varying])::text[]))`),
	check("check_initial_level", sql`(initial_level IS NULL) OR ((initial_level)::text = ANY ((ARRAY['A1'::character varying, 'A2'::character varying, 'B1'::character varying, 'B2'::character varying, 'C1'::character varying, 'C2'::character varying])::text[]))`),
	check("check_level_status", sql`(level_status IS NULL) OR ((level_status)::text = ANY ((ARRAY['confirmed'::character varying, 'provisional'::character varying, 'pending_approval'::character varying])::text[]))`),
]);
...
export const classes = pgTable("classes", {
	...
	index("idx_classes_dates").using("btree", table.startDate.asc().nullsLast().op("date_ops"), table.endDate.asc().nullsLast().op("date_ops")),
...
export const classSessions = pgTable("class_sessions", {
	...
	index("idx_sessions_class_date").using("btree", table.classId.asc().nullsLast().op("date_ops"), table.sessionDate.asc().nullsLast().op("date_ops")),
	foreignKey({
...
export const attendance = pgTable("attendance", {
	...
	uniqueIndex("idx_attendance_session_student").using("btree", table.classSessionId.asc().nullsLast().op("uuid_ops"), table.studentId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
...
export const enrollments = pgTable("enrollments", {
	...
	uniqueIndex("idx_enrollments_student_class").using("btree", table.studentId.asc().nullsLast().op("uuid_ops"), table.classId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
...
export const enrollmentAmendments = pgTable("enrollment_amendments", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	tenantId: uuid("tenant_id").notNull(),
	enrollmentId: uuid("enrollment_id").notNull(),
	amendmentType: varchar("amendment_type", { length: 50 }).notNull(),
	amendmentDate: date("amendment_date").default(sql`CURRENT_DATE`).notNull(),
	...
	index("idx_enrollment_amendments_date").using("btree", table.amendmentDate.asc().nullsLast().op("date_ops")),
	index("idx_enrollment_amendments_enrollment").using("btree", table.enrollmentId.asc().nullsLast().op("uuid_ops")),
	index("idx_enrollment_amendments_status").using("btree", table.status.asc().nullsLast().op("text_ops")),
	index("idx_enrollment_amendments_tenant").using("btree", table.tenantId.asc().nullsLast().op("uuid_ops")),
	index("idx_enrollment_amendments_type").using("btree", table.amendmentType.asc().nullsLast().op("text_ops")),
	foreignKey({
	...
	pgPolicy("enrollment_amendments_insert_admin", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`((tenant_id = (current_setting('app.current_tenant_id'::text))::uuid) AND (current_setting('app.current_user_role'::text) = ANY (ARRAY['admin'::text, 'super_admin'::text])))`  }),
	pgPolicy("enrollment_amendments_select_admin", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("enrollment_amendments_select_student", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("enrollment_amendments_update_admin", { as: "permissive", for: "update", to: ["public"] }),
	check("check_amendment_type", sql`(amendment_type)::text = ANY ((ARRAY['extension'::character varying, 'reduction'::character varying, 'transfer'::character varying, 'level_change'::character varying, 'cancellation'::character varying])::text[])`),
]);
...
export const lessonPlans = pgTable("lesson_plans", {
	...
	index("idx_lesson_plans_teacher").using("btree", table.teacherId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
...
export const vStudentsWithMetadata = pgView("v_students_with_metadata", {	id: uuid(),
...
export const vStudentCourseHistory = pgView("v_student_course_history", {	studentId: uuid("student_id"),
	...
	attendanceRate: numeric("attendance_rate", { precision: 5, scale:  2 }),
	currentGrade: varchar("current_grade", { length: 10 }),
}).as(sql`SELECT u.id AS student_id, u.name AS student_name, u.email AS student_email, e.id AS enrollment_id, e.enrollment_date, e.completion_date, e.status AS enrollment_status, c.id AS class_id, c.name AS class_name, c.level AS class_level, c.start_date AS class_start_date, c.end_date AS class_end_date, e.attendance_rate, e.current_grade FROM users u JOIN enrollments e ON e.student_id = u.id JOIN classes c ON c.id = e.class_id WHERE u.role::text = 'student'::text AND c.deleted_at IS NULL ORDER BY u.name, e.enrollment_date DESC`);