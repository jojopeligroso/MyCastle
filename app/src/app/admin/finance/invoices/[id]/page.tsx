...
async function getInvoicePayments(invoiceId: string) {
  const invoicePayments = await db
    ...
    .from(payments)
    .leftJoin(users, eq(payments.recorded_by, users.id))
...
export default async function InvoiceDetailPage({
  ...
  const result = await getInvoice(params.id, tenantId);
              ...
              <div className="overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {invoicePayments.map(({ payment, recorded_by_user }) => (
                      <tr key={payment.id}>
                        <td className="px-4 py-3 text-sm text-gray-900">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="space-y-2">
              <Link
                href={`/admin/finance/payments?invoice=${params.id}`}
