...
export default async function AttendanceDetailPage({ params }: PageProps) {
    ...
    const enrolledStudents = await db
        ...
        .where(and(
            eq(enrollments.class_id, session.classId),
            ...
            <div className="bg-white shadow sm:rounded-lg p-6">
                <div className="md:flex md:items-center md:justify-between">
                    <div className="flex-1 min-w-0">
                        <Link href="/admin/attendance" className="text-sm text-purple-600 hover:text-purple-800 mb-2 inline-block">
                            &larr; Back to Dashboard
                        </Link>
                        <h2 className="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">
                            {session.className} <span className="text-gray-500 font-normal">({session.classCode})</span>
                        </h2>
