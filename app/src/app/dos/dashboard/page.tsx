  ...
  const [approvedThisMonth] = await db
    ...
    .where(
      and(
        ...
        gte(levelPromotions.reviewedAt, new Date(new Date().getFullYear(), new Date().getMonth(), 1))
      )
    );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <Link href="/dos/students" className="mt-4 text-sm text-purple-600 hover:text-purple-700 flex items-center">
            View all students
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <p className="mt-4 text-sm text-gray-500">
            {Math.round((data.counts.activeStudents / (data.counts.totalStudents || 1)) * 100)}% of total
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg border border-amber-200 p-6 bg-amber-50">
          ...
          <Link href="/admin/promotions" className="mt-4 text-sm text-amber-700 hover:text-amber-800 flex items-center font-medium">
            Review now
            <svg className="w-4 h-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded ${getLevelBadgeColor(promotion.fromLevel)}`}>
                          {promotion.fromLevel}
                        </span>
                        <svg className="w-3 h-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
                        </svg>
                        <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded ${getLevelBadgeColor(promotion.toLevel)}`}>
                          {promotion.toLevel}
                        </span>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-medium text-gray-900">
                        {promotion.summativeAvg !== null ? `${promotion.summativeAvg.toFixed(1)}%` : 'N/A'}
                      </p>
